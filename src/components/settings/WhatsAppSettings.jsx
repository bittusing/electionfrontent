...
export default function WhatsAppSettings() {
  ...
  const fields = {
    twilio: [
      { name: 'accountSid', label: 'Account SID', type: 'text', required: true, sensitive: true },
      { name: 'authToken', label: 'Auth Token', type: 'password', required: true, sensitive: true },
      { name: 'phoneNumber', label: 'Phone Number', type: 'text', required: true, placeholder: '+1234567890', hint: 'Use E.164 format' }
    ],
    gupshup: [
    ...
    'whatsapp-business': [
      { name: 'phoneNumberId', label: 'Phone Number ID', type: 'text', required: true, sensitive: true },
      { name: 'businessAccountId', label: 'Business Account ID', type: 'text', required: true, sensitive: true },
  ...
  const loadCredentials = async () => {
    try {
      setLoading(true)
      const response = await api.get('/messaging-settings/credentials/whatsapp')
      setCredentials(response.data.data || [])
      
      // Load stats for each credential
  ...
  const loadStats = async (provider) => {
    try {
      const response = await api.get(`/messaging-settings/credentials/whatsapp/${provider}/stats`)
      setStats(prev => ({
        ...prev,
  ...
  const handleTest = async (provider, credentials) => {
    try {
      const response = await api.post('/messaging-settings/credentials/whatsapp/test', {
        provider,
        credentials
      })
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-gray-600 mt-1">
            Configure WhatsApp Business API credentials for sending messages
          </p>
        </div>
        <button
      ...
      {credentials.length === 0 && !showAddForm ? (
        <div className="card text-center py-12">
          ...
          <h3 className="text-lg font-medium text-gray-800 mb-2">
            No WhatsApp Credentials Configured
          </h3>
          <p className="text-gray-600 mb-4">
            Add your first WhatsApp provider to start sending messages
          </p>
          <button

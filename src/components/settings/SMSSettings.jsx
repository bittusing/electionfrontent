...
export default function SMSSettings() {
  ...
  const fields = {
    ...
    msg91: [
      { name: 'authKey', label: 'Auth Key', type: 'password', required: true, sensitive: true },
      { name: 'senderId', label: 'Sender ID', type: 'text', required: true, hint: 'Max 6 characters' }
    ],
    textlocal: [
  ...
  const loadCredentials = async () => {
    try {
      setLoading(true)
      const response = await api.get('/messaging-settings/credentials/sms')
  ...
  const loadStats = async (provider) => {
    try {
      const response = await api.get(`/messaging-settings/credentials/sms/${provider}/stats`)
      setStats(prev => ({
        ...prev,
  ...
  const handleTest = async (provider, credentials) => {
    try {
      const response = await api.post('/messaging-settings/credentials/sms/test', {
        provider,
        credentials
      })
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-xl font-semibold text-gray-800">SMS Configuration</h2>
          <p className="text-sm text-gray-600 mt-1">
            Configure SMS API credentials for sending text messages
          </p>
        </div>
        <button
      ...
      {credentials.length === 0 && !showAddForm ? (
        <div className="card text-center py-12">
          ...
          <p className="text-gray-600 mb-4">
            Add your first SMS provider to start sending messages
          </p>
          <button
